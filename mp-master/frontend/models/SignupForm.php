...
class SignupForm extends Model
{
    ...
    public function overlyNumeric($attribute, $params)
    {
      if (preg_match_all( "/[0-9]/", $this->$attribute)>4) {
        $this->addError($attribute, Yii::t('frontend','Sorry, we do not support your email address.'));
      }
    }

    public function overlyPeriod($attribute, $params)
    {
      if (substr_count($this->$attribute,'.')>4) {
        $this->addError($attribute, Yii::t('frontend','Sorry, we do not support your email address.'));
      }
    }
