...
class Meeting extends \yii\db\ActiveRecord
{
  ...
  const COMMAND_USER_VERIFY_EMAIL = 465;
  const COMMAND_RESPOND_MESSAGE = 470;
  const COMMAND_DOWNLOAD = 480;
    ...
    public function getMeetingTypeOptions()
    {
     ...
     public function getMeetingParticipants($user_id=0, $prefix = true, $exclude_owner = false) {
       ...
       if (count($this->participants)>0) {
        ...
        return (($prefix && strlen($str)>0)?Yii::t('frontend','with').' '.$str:$str);
       }
     }
    ...
    foreach ($attendees as $cnt=>$a) {
      ...
      if (User::checkEmailDelivery($a['user_id'],$user_id)) {
        ...
        $links=[
          ...
          'download'=>MiscHelpers::buildCommand($this->id,Meeting::COMMAND_DOWNLOAD,0,$a['user_id'],$a['auth_key'],$this->site_id),
      ...
      public function beforeSave($insert)
      {
       ...
       public static function prepareCalendarLink($meeting_id,$actor_id,$calendar) {
         // to do - unify this code with prepareDownloadIcs
         ...
         $auth_key=\common\models\User::find()->where(['id'=>$actor_id])->one()->auth_key;
         $replaceUrl =Url::to(['']);
         $participantStr=$m->getMeetingParticipants();
         date_default_timezone_set( "UTC" );
         switch ($calendar) {
             case 'google':
             // to do - build the array first
             $propertyArray = [
               '',
               'action'=>'TEMPLATE',
               'text'=>$header,
               'dates'=>gmdate('Ymd\THis\Z',$chosenTime->start)."/".gmdate('Ymd\THis\Z',$chosenTime->end),
               'details'=>($m->message==''?'':$m->message.' ').($participantStr==''?'':Yii::t('frontend','Meeting').' '.$participantStr.'. ').Yii::t('frontend','View or make changes to the meeting invitation').': '.MiscHelpers::buildCommand($m->id,Meeting::COMMAND_VIEW,0,$actor_id,$auth_key,$m->site_id),
               'sf'=>'true',
               'output'=>'xml',
             ];
             // if place
             if (!$noPlaces) {
               $propertyArray['location']=$chosenPlace->place->name.','.$chosenPlace->place->full_address;
               $propertyArray['sprop']='website:'.$chosenPlace->place->website;
             }
             // if participants
             // disabled for now to gather feedback. If you add the emails, google calendar will start emailing people
             //$propertyArray['add']='j7@email.com';
              $url =Url::to($propertyArray);
               $url = str_replace($replaceUrl,'https://calendar.google.com/calendar/render',$url);
             break;
           break;
           case 'yahoo':
            // via http://taskboy.com/blog/Creating_events_for_Yahoo_and_Google_calendars.html
            $propertyArray = [
              '',
              'TITLE'=>$header,
              'v'=>60,
              'ST'=>gmdate('Ymd\THis\Z',$chosenTime->start),
              'DUR'=>gmdate('Hi', $chosenTime->duration*60),
              'DESC'=>($m->message==''?'':$m->message.' ').($participantStr==''?'':Yii::t('frontend','Meeting').' '.$participantStr.'. '.Yii::t('frontend','View or make changes to the meeting invitation').': '.MiscHelpers::buildCommand($m->id,Meeting::COMMAND_VIEW,0,$actor_id,$auth_key,$m->site_id)),
              'URL'=>MiscHelpers::buildCommand($m->id,Meeting::COMMAND_VIEW,0,$actor_id,$auth_key,$m->site_id),
            ];
            // if place
            if (!$noPlaces) {
              $propertyArray['in_loc']=$chosenPlace->place->name;
              //$propertyArray['in_st'] $chosenPlace->place->full_address;
              //$propertyArray['csz']
            }
            $url =Url::to($propertyArray);
             $url = str_replace($replaceUrl,'http://calendar.yahoo.com/',$url);
           break;
         }
         return $url;
       }
       ...
       public static function prepareDownloadIcs($meeting_id,$actor_id) {
         // to do - unify this code with preparecalendarlink
         $m = Meeting::findOne($meeting_id);
         // Get message header
         $header = $m->getMeetingHeader();
         // chosen time
         ...
         foreach ($attendees as $cnt=>$a) {
           if ($a['user_id']==$actor_id) {
             ...
             if (MiscHelpers::isMacSafari()) {
                Yii::$app->response->redirect(MiscHelpers::buildUrl().'/'.$icsPath);
             } else if (!MiscHelpers::isIphone()) {
               //echo MiscHelpers::buildUrl().'/'.$icsPath;
       ...
       if (User::checkEmailDelivery($a['user_id'],$user_id)) {
         ...
         Yii::$app->timeZone = $timezone = MiscHelpers::fetchUserTimezone($a['user_id']);
           ...
           $links=[
             ...
             'footer_block_all'=>MiscHelpers::buildCommand($mtg->id,Meeting::COMMAND_FOOTER_BLOCK_ALL,0,$a['user_id'],$a['auth_key'],$mtg->site_id),
             'add_contact'=>MiscHelpers::buildCommand($mtg->id,Meeting::COMMAND_ADD_CONTACT,0,$a['user_id'],$a['auth_key'],$mtg->site_id)
           ];
           // send the message
           $message = Yii::$app->mailer->compose([
             'html' => 'contact-html',
