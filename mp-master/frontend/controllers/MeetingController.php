...
class MeetingController extends Controller
{
          ...
          'access' => [
                        ...
                        'rules' => [
                          // allow authenticated users
                           [
                               'allow' => true,
                               'actions'=>['create','createactivity','index','view','viewplace','removeplace','viewactivity','removeactivity','update','delete', 'decline','cancel','cancelask','command','download','downloadask','trash','late','cansend','canfinalize','send','finalize','virtual','reopen','reschedule','repeat','resend','identity','updatewhat','scheduleme'],
                               'roles' => ['@'],
                           ],
                          [
                              'allow' => true,
    ...
    public function actionIndex()
    {
      ...
      $planningProvider = new ActiveDataProvider([
        'query' => Meeting::find()
          ...
          ->where(['owner_id'=>Yii::$app->user->getId(),'meeting.status'=>[Meeting::STATUS_PLANNING,Meeting::STATUS_SENT]])
          ->orWhere(['participant_id'=>Yii::$app->user->getId(),'meeting.status'=>Meeting::STATUS_SENT])
          ->distinct(),
    ...
    public function actionDownloadask($id,$actor_id) {
      // display button to choose calendar to download or link event to
      if ($actor_id == Yii::$app->user->getId() && Meeting::isAttendee($id,$actor_id)) {
        $model = Meeting::findOne($id);
        return $this->render('downloadask', [
          'model'=>$model,
          'id'=>$id,
        ]);
      }
    ...
    public function actionDownload($id,$actor_id=false,$calendar='apple') {
      // sometimes user arrives from _grid w/o actor_id or other times from email
      if ($actor_id===false) {
        $actor_id = Yii::$app->user->getId();
      }
      if ($actor_id == Yii::$app->user->getId() && Meeting::isAttendee($id,$actor_id)) {
        if ($calendar=='apple' || $calendar=='outlook') {
          Meeting::prepareDownloadIcs($id,$actor_id);
        } else {
          $url = Meeting::prepareCalendarLink($id,$actor_id,$calendar);
          $this->redirect($url);
        }
      }
    }
      ...
      if (!$authResult) {
        ...
        switch ($cmd) {
          ...
          case Meeting::COMMAND_VERIFY_EMAIL:
            // verifies the email for meeting planner
            $u  = \common\models\User::findOne($actor_id);
            $u->status = \common\models\User::STATUS_ACTIVE;
            // to do - UserVerify add row
            $u->update();
            $this->redirect(['meeting/view','id'=>$id]);
          break;
          case Meeting::COMMAND_USER_VERIFY_EMAIL:
            // distinctly verifies user email
            $u  = \common\models\User::findOne($actor_id);
            $u->status = \common\models\User::STATUS_ACTIVE;
            $u->update();
            $uv = \frontend\models\UserVerify::checkVerification($u->id,$obj_id);
            $this->goHome();
          break;
          ...
          case Meeting::COMMAND_RESPOND_MESSAGE:
            ...
            break;
          case Meeting::COMMAND_DOWNLOAD:
            $this->redirect(['meeting/downloadask','id'=>$id,'actor_id'=>$actor_id]);
            break;
